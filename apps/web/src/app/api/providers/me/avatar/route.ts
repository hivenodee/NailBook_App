...
// POST /api/providers/me/avatar — get presigned upload URL for avatar
export async function POST(request: NextRequest) {
  ...
  const key = `avatars/${user.provider.id}/${randomUUID()}.${ext}`;
  ...
  return success({ uploadUrl, avatarUrl: publicUrl }, 201);
}
